...
@fragment fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  var level = 1.0;
  // Match the reference: subdued shapes behind the title, brighter toward the bottom.
  level *= mix(0.035, 0.82, smoothstep(0.0, 1.0, uv.y));
...
export default function ShapeWaves({
  ...
  useEffect(() => {
    ...
    // Preserve the submitted WebGPU frame before its drawing buffer is recycled.
    const lensSource = document.createElement('canvas');
    lensSource.className = 'shape-waves__lens-source';
    lensSource.hidden = true;
    lensSource.style.setProperty('display', 'none', 'important');
    root?.querySelector('.shape-waves__lens-source')?.remove();
    root?.appendChild(lensSource);
    const lensContext = lensSource.getContext('2d');
    ...
    void (async () => {
      try {
        ...
        const render = now => {
          ...
          if (lensContext) {
            if (lensSource.width !== canvas.width || lensSource.height !== canvas.height) {
              lensSource.width = canvas.width;
              lensSource.height = canvas.height;
            }
            lensContext.drawImage(canvas, 0, 0);
            lensSource.dataset.frameReady = 'true';
          }
          if (!presented) {
            presented = true;
            setReady(true);
