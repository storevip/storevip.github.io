// React Bits OptionWheel — https://github.com/DavidHDev/react-bits
...
const OptionWheel = ({
  ...
  const handleItemClick = useCallback(
    index => {
      ...
      if (index === selectedRef.current) onActivate?.(index, cfg.items[index]);
      else applyTarget(cur + d, true);
    },
  ...
  const handleKeyDown = useCallback(
    e => {
      if (e.key === 'Enter' || e.key === ' ') {
        e.preventDefault();
        onActivate?.(selectedRef.current, cfgRef.current.items[selectedRef.current]);
        return;
      }
      let delta = null;
